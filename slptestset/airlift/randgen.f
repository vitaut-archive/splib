c     This file makes (non-block) indep vars, which is not what Midler
c     and Wollmer did, I think.  But it makes an interesting case, since
c     simplex/simplex takes 14 iterations
...
C     uniran is a function which returns a U(0,1) random number
      double precision Xbar, SV, Sum1, Sum2, n
	    ...
	    double precision mu,sigma, Lmu, Lsigma, Cmu, Csigma, prob
	    integer m, count
      ...
      DO WHILE (count.lt.n) 
         ...
         If(W.le.1) then
            ...
            X2=mu+V2*sigma*SQRT(-2*LOG(W)/W)   
	          LX1 = ((X1 - Cmu)/Csigma)* Lsigma + Lmu
            LX2 = ((X2 - Cmu)/Csigma)* Lsigma + Lmu
            write (3,2000) 'RIGHT   ','DEMAND',m,LX1,'PERIOD2',prob
            write (3,2000) 'RIGHT   ','DEMAND',m,LX2,'PERIOD2',prob
 2000       format(4x,a8,2x,a6,i1.1,3x,f12.6,2x,a8,2x,f6.3)
